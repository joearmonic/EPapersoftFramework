...
namespace EPapersoftFramework.DAL.TestConsole
{
    internal class Program
    {
        ...
        private static IRepository<Dwelling> dwellingRepo = null;
        private static IRepository<Resident> residentRepo = null;
        private static TransactionKeeper tranKeeper = null;
        private static IDataProviderFactory dataFactory;
        private static TableMappingFactory pnc6MappingFactory;

        private static void Main(string[] args)
        {
            WriteColoredLine(Assembly.GetExecutingAssembly().GetName().Name + " " + Assembly.GetExecutingAssembly().GetName().Version, ConsoleColor.Gray);
            /* Repo only needs
             * a connection string
             * and the full qualified name where to load the factory for the db engine (could be by config).
             */
        ...
        private static ITransactionService EstablishMode()
        {
            ...
            if (_mode == 2)
            {
                trans = dataFactory.CreateTransactionService(pnc6MappingFactory);
                dwellingRepo = trans.RegisterRepository<Dwelling>(dwellingRepo);
                residentRepo = trans.RegisterRepository<Resident>(residentRepo);
            }
        ...
        {
            dataFactory = new SqlProviderFactory(ConfigurationManager.ConnectionStrings["PNC7"].ConnectionString);
            // new SAProviderFactory(ConfigurationManager.ConnectionStrings["PNC"].ConnectionString);
            pnc6MappingFactory = new PNC7MappingFactory();
            // new PNC6MappingFactory();

            dwellingRepo = new ADONetRepository<Dwelling>
           (
               dataFactory
               , pnc6MappingFactory
            );

            residentRepo = new ADONetRepository<Resident>
          (
              dataFactory
              , pnc6MappingFactory
           );
        ...
        private static void ExecuteCommand(string command)
        {
            ...
            else if (command.ToUpper().StartsWith("GETALL"))
            {
                ...
                {
                    if (cmdWithArgs[1].Equals(dwellingRepo.GetType().GetGenericArguments()[0].Name))
                    {
                        var entities = SelectEntities(dwellingRepo);
                        foreach (var entity in entities)
                        {
                            RenderEntityDef(entity, "LocationDef");
                        }
                    }
                    else if (cmdWithArgs[1].Equals(residentRepo.GetType().GetGenericArguments()[0].Name))
                    {
                        var entities = SelectEntities(residentRepo);
                        foreach (var entity in entities)
                        {
                            RenderEntityDef(entity, "ResidentDef");
                        }
                    }
                }
            }
            else if (Int32.TryParse(command, out id))
            {
                try
                {
                    ...
                    {
                        case 1:
                            EntityOption(dwellingRepo);
                            break;

                        case 2:
                            EntityOption(residentRepo);
                            break;
        ...
        {
            WriteColoredLine("Choose which entity you want to load or write one command:");
            WriteColoredLine(" 1. Dwelling", ConsoleColor.DarkYellow);
            WriteColoredLine(" 2. Resident", ConsoleColor.DarkYellow);
        ...
        private static T SelectEntity<T>(IRepository<T> repo, ref string command) where T : class
        {
            ...
            Console.WriteLine("ENTITY's PRIMARY DEF to operate with?");
